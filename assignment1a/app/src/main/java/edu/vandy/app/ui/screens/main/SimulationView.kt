...
class SimulationView @JvmOverloads constructor(
    ...
    private var marginRange =
        Range(
            4.dpToPx.toInt() + PROGRESS_SIZE.height,
...
//        setLayerType(LAYER_TYPE_SOFTWARE, palantiriPaint)
    ...
    internal fun updateModel(snapshot: ModelSnapshot) {
        ...
        if (Settings.modelChecker) {
            ...
            try {
                ...
                context.toast(
                    "The currently Palantir model has not " +
        ...
        if (Settings.beingSizeRange.max < snapshot.beings.count()) {
            context.toast(
                "Current display size and resolution " +
    ...
    private fun calcMaxSpriteCounts(portraitMode: Boolean)
        ...
        println(
            "calcMaxSpriteCounts: portraitSize = $portraitSize " +
     ...
     * landscape modes.
     */
    private fun calcMaxSpriteSizes(
        layout: Size,
        minMargin: Int
    )
     ...
     * variables.
     */
    private fun calcSpriteSizeRange(
        layoutSize: Int,
     ...
     * [heightRange].
     */
    private fun heightRangeToWidthRange(
        sprites: Map<Long, Sprite>,
        heightRange: Range<Int>
    ): Range<Int> {
        val minSize = calcMaxSpriteSize(
            sprites,
            heightRange.lower.toFloat()
        )
        val maxSize = calcMaxSpriteSize(
            sprites,
            heightRange.upper.toFloat()
        )
        return Range(
            minSize.width.roundToInt(),
     ...
     * [widthRange].
     */
    private fun widthRangeToHeightRange(
        sprites: Map<Long, Sprite>,
        ...
        return Range(
            (widthRange.lower * aspect).roundToInt(),
    ...
    private fun validateBeingLayout(bounds: Size) {
        ...
        if (landscape) {
            val minWidth =
                calcMaxSpriteSize(
                    beings, minSize.toFloat()
                ).width.toInt()
            val curWidth =
                calcMaxSpriteSize(
                    beings, curSize.toFloat()
                ).width.toInt()

            validateMinMaxLayouts(
                "Being",
                bounds,
                curCount,
                curWidth,
    ...
    private fun validatePalantirLayout(bounds: Size) {
        ...
        if (landscape) {
            val minWidth =
                calcMaxSpriteSize(
                    palantiri, minSize.toFloat()
                ).width.toInt()
            val curWidth =
                calcMaxSpriteSize(
                    palantiri, curSize.toFloat()
                ).width.toInt()
    ...
    private fun validateMinMaxLayouts(
        name: String,
        bounds: Size,
        curCount: Int,
        curSize: Int,
    ...
    abstract class Sprite(val view: SimulationView) : Drawable(), KtLogger {
        ...
        override fun draw(canvas: Canvas) {
            ...
            if (Settings.showSprites) {
                ...
                matrix.postScale(
                    drawRect.width().toFloat() / bitmap.width,
     ...
     * at draw time based on the current snapshot property.
     */
    class PalantirSprite(
        view: SimulationView,
        snapshot: PalantirSnapshot
    ) : Sprite(view) {
        /**
        ...
        override val stateName
            get() = when (state) {
                ...
                else -> PalantirComponent.State.AVAILABLE.name.lowercase()
            }
