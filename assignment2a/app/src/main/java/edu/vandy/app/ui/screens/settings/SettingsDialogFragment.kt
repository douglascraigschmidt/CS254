...
import edu.vandy.app.ui.screens.settings.Settings.threadCountRange
import edu.vandy.app.ui.screens.settings.adapters.BeingManagerTypeSpinnerAdapter
import edu.vandy.app.ui.screens.settings.adapters.PalantiriManagerTypeSpinnerAdapter
...
import edu.vandy.databinding.SettingsDialogFragmentBinding
import edu.vandy.simulator.managers.beings.BeingManager
import edu.vandy.simulator.managers.beings.BeingManager.Factory.Type.*
import edu.vandy.simulator.managers.palantiri.PalantiriManager
import edu.vandy.simulator.managers.palantiri.PalantiriManager.Factory.Type.ARRAY_BLOCKING_QUEUE
import edu.vandy.simulator.managers.palantiri.PalantiriManager.Factory.Type.SUSPENDING_SPIN_LOCK_SEMAPHORE
...
class SettingsDialogFragment :
    ...
    private val threadCountTunable: Boolean
        get() = Settings.beingManagerType == STRUCTURED_CONCURRENCY
                && Settings.palantirManagerType == SUSPENDING_SPIN_LOCK_SEMAPHORE
                || Settings.beingManagerType == EXECUTOR_SERVICE

    private fun updateThreadCountRange(max: Int) {
        val keepAtMax = Settings.threadCount == threadCountRange.max
        threadCountRange = threadCountRange.setMax(max)
        if (keepAtMax || Settings.threadCount > threadCountRange.max) {
            Settings.threadCount = threadCountRange.max
        }
    ...
    private fun updateThreadCount() {
        val enabled = !simulationRunning && threadCountTunable

        with(binding) {
            settingsThreadCountLabel.enable(enabled)
            settingsThreadCountLayout.enable(enabled)
            settingsThreadCountSeekBar.show = threadCountTunable
        }

        configureThreadCount()
    }

    private fun onBeingManagerChanged() {
        when (Settings.beingManagerType) {
            STRUCTURED_CONCURRENCY -> {
                if (Settings.palantirManagerType != SUSPENDING_SPIN_LOCK_SEMAPHORE) {
                    Settings.palantirManagerType = SUSPENDING_SPIN_LOCK_SEMAPHORE
                }
            }
            else -> {
                if (Settings.palantirManagerType == SUSPENDING_SPIN_LOCK_SEMAPHORE) {
                    Settings.palantirManagerType = ARRAY_BLOCKING_QUEUE
                }
            }
        }
    ...
    private fun onPalantiriManagerChanged() {
        when (Settings.palantirManagerType) {
            SUSPENDING_SPIN_LOCK_SEMAPHORE -> {
                if (Settings.beingManagerType != STRUCTURED_CONCURRENCY) {
                    Settings.beingManagerType = STRUCTURED_CONCURRENCY
                }
            }
            else -> {
                if (Settings.beingManagerType == STRUCTURED_CONCURRENCY) {
                    Settings.beingManagerType = RUNNABLE_THREADS
                }
            }
        }
    ...
    override fun onSharedPreferenceChanged(
        ...
        with(Settings) {
            when (key) {
                ...
                SIMULATION_THREAD_COUNT_PREF -> configureThreadCount()
    ...
    private fun configureBeingManagerType(update: Boolean = false) {
        with(binding) {
            if (update) {
                ...
                val adapter = settingsBeingManagerType.adapter as BeingManagerTypeSpinnerAdapter
                settingsBeingManagerType.setSelection(
                    adapter.getPositionForValue(Settings.beingManagerType) ?: 0
                )
            } else {
                // Setup adapter and current selection.
                val adapter = BeingManagerTypeSpinnerAdapter(
    ...
    private fun configurePalantiriManagerType() {
        // Setup adapter and current selection.
        val adapter = PalantiriManagerTypeSpinnerAdapter(
            ContextThemeWrapper(activity, R.style.SettingsThemeDark),
        ...
        with(binding) {
            ...
            settingsPalantiriManagerType.setSelection(
                adapter.getPositionForValue(Settings.palantirManagerType) ?: 0
            )
            ...
            compositeDisposable.add(
                RxSeekBar.userChanges(settingsBeingsSeekBar)
                    ...
                    .subscribe {
                        ...
                        updateThreadCountRange(value)
                    })
        }
    }

    private fun configureThreadCount() {
        with(binding) {
            settingsThreadCountSeekBar.grabTouchEvents()

            // Ensure that thread count is never more than the current being count.
            val beingCount = Settings.beingCount
            settingsThreadCountSeekBar.max = threadCountRange.toOffset(beingCount)

            if (Settings.threadCount > beingCount) {
                updatePreference { Settings.threadCount = beingCount }
            } else if (!threadCountTunable && Settings.threadCount < beingCount) {
                updatePreference { Settings.threadCount = beingCount }
            }

            settingsThreadCountSeekBar.progress =
                threadCountRange.toOffset(Settings.threadCount)
            settingsThreadCountValue.text =
                String.format("%3d", Settings.threadCount)

            compositeDisposable.add(
                RxSeekBar.userChanges(settingsThreadCountSeekBar)
                    .skipInitialValue()
                    .subscribe {
                        val value = threadCountRange.fromOffset(it)
                        updatePreference { Settings.threadCount = value }
                        settingsThreadCountValue.text = String.format("%3d", value)
                    })
        }
    }
    ...
    private var ignoreGazingEvent = false
