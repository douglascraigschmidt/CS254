...
class Assignment_3B_FairSemaphoreWhiteBoxCOTest : AssignmentTests(0) {
    @Mock
    ...
    @After
    fun after() {
        // Make sure thread interrupt flag is always cleared after each test.
        Thread.currentThread().isInterrupted
    }

    @Test
    fun `test constructor`() {
        with(FairSemaphoreCO(9)) {
            assertThat(primitiveValueHasModifier(java.lang.reflect.Modifier.VOLATILE, Int::class)).isTrue
            assertThat(primitiveValueHasModifier(java.lang.reflect.Modifier.STATIC, Int::class)).isFalse
            assertThat(value<Lock>() is ReentrantLock).isTrue
            assertThat((value<Lock>() as ReentrantLock).isFair).isFalse
    ...
    fun testFairSemaphoreFields() {
        ...
        assertTrue(
            ...
            "FairSemaphoreCO class Lock field should be set to a non-null ReentrantLock instance."
        )
