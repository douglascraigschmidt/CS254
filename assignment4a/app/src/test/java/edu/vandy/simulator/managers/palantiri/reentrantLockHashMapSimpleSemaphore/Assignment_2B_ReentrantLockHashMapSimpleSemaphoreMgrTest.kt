...
class Assignment_2B_ReentrantLockHashMapSimpleSemaphoreMgrTest : AssignmentTests() {
    ...
    fun `build model undergraduate and graduate test`() {
        ...
        assertEquals(
            "getPalantiriMap() should contain $PALANTIR_COUNT entries.",
    ...
    fun `build model graduate only test`() {
        ...
        val mockMap = mockk<Map<Palantir, Boolean>>()
        mockMap.injectInto(manager, Map::class.java)
        ...
        assertThat(manager.value<Map<Palantir, Boolean>>()).isNotSameAs(mockMap)
    ...
    fun `acquire all available palantiri graduate version`() {
        ...
        val entry = mockk<MutableMap.MutableEntry<Palantir, Boolean>>()
        every { entry.setValue(any()) } answers {
            true
        }
        every { entry.value } answers {
            fail("Do not call Map.Entry<>.getValue()")
            false
        }
        ...
        every {
            ...
            arg<Predicate<Map.Entry<Palantir, Boolean>>>(0).test(entry)
            streamEntrySet
        }
    ...
    fun `acquire does not call unlock if semaphore acquire fails`() {
        every { semaphore.acquire() } throws AssignmentTests.SimulatedException()
    ...
    fun `release all acquired palantiri`() {
        ...
        assertEquals(
            "All $PALANTIR_COUNT Palantiri should be unlocked.",
