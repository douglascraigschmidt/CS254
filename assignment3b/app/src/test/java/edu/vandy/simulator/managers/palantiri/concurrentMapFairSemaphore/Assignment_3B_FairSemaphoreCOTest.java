...
import static edu.vandy.simulator.utils.Student.Type.Graduate;
...
/**
 * Unit test for the FairSemaphoreMo.
 */
public class Assignment_3B_FairSemaphoreCOTest extends AssignmentTests {
    /**
    ...
    public void testFairSemaphore() {
        FairSemaphore fairSemaphore = new FairSemaphoreCO(2);
        assertEquals(2, fairSemaphore.availablePermits());
    }
    ...
    public void testNegativePermits() throws InterruptedException {
        FairSemaphore fairSemaphore = new FairSemaphoreCO(-1);
        assertEquals(fairSemaphore.availablePermits(), -1);
        fairSemaphore.release();
        assertEquals(fairSemaphore.availablePermits(), 0);
        fairSemaphore.release();
        ...
        FairSemaphore fairSemaphore2 = new FairSemaphoreCO(-1);
    ...
    public void testConcurrentAccess() {
        ...
        FairSemaphore semaphore = new FairSemaphoreCO(PERMIT_COUNT);
            ...
            Thread t = new Thread(() -> {
                ...
                for (int j = 0; j < ACCESS_COUNT; ++j) {
                    ...
                    try {
                        ...
                        log("In testConcurrentAccess(), where thread "
    ...
    public void testFairness() throws InterruptedException {
        ...
        FairnessChecker checker = new FairnessChecker(MAX_THREADS);
            ...
            Thread t = new Thread(() -> {
                for (int j = 0; j < (MAX_THREADS - 1); ++j) {
                    ...
                    try {
                        ...
                        log("In testFairness(), where thread "
                                + Thread.currentThread().getId()
                                + " got an InterruptedException");
                    }
            ...
            Thread t = new Thread(() -> {
                for (int j = 0; j < (MAX_THREADS - 1); ++j) {
                    try {
                        ...
                        mInterrupted = true;
                        log("In testFairnessWithInterrupts(), where thread "
                                + Thread.currentThread().getId()
                                + " got an InterruptedException");
                        return;
                    }
